...
out_path = os.path.join(plot_dir, "Box_BH.png")

df = pd.read_csv(r"E:\UHAE\BH&BD\city_BH.csv")  # city_name, BH_2010, BH_2020, size
...
df_long = pd.melt(df_plot,
                  ...
                  value_vars=["BH_2010", "BH_2020"],
                  var_name="Year",
                  value_name="BH_Value")
df_long["BH_Value"] = df_long["BH_Value"] # * 100
# 设置 size 顺序和颜色
...
for i, size in enumerate(size_order):
    for j, year in enumerate(["BH_2010", "BH_2020"]):
        # 箱子横坐标
        x_pos = i + (-0.5 + j) * offset * 2  # 两年份微偏移

        # 对应数据
        y_vals = df_long[(df_long["size"] == size) & (df_long["Year"] == year)]["BH_Value"].dropna()
        if len(y_vals) == 0:
            continue
...
ax.set_ylabel("Building Height(m)", fontsize=16)
