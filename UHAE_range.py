...
df['marker_size'] = df['size_2'].map(size_map)
'''
# 提取字符串区间中的最大值
def extract_max(s):
    if pd.isna(s):
...
for col in y_cols:
    ...
    size_category_all = df['size_2'][valid_mask_all]
    print(f"共有 {len(x_valid_all)} 条数据参与建模")
    # === 所有参与回归散点的平均值及 95% 置信区间 ===
    n_all = len(y_valid_all)
    mean_all = np.mean(y_valid_all)
    std_all = np.std(y_valid_all, ddof=1)  # 样本标准差
    se_all = std_all / np.sqrt(n_all)  # 标准误
    t_crit_all = t.ppf(0.975, n_all - 1)  # 95% CI
    ci_lower = mean_all - t_crit_all * se_all
    ci_upper = mean_all + t_crit_all * se_all

    print(
        f"参与回归散点的平均值 = {mean_all:.4f}, "
    ...
    for category in ['small', 'middle', 'large', 'all']:
        ...
        info = f"{category.capitalize()} R²={r_squared:.3f},  p={p_value:.3g}"
    ...
    #plt.ylabel(f"Spatial extent of UHA effect (km) ")
    # plt.title(f'Linear Regression: Thermal lag range vs Center BH Rise')
    plt.legend(loc='upper left', bbox_to_anchor=(0, 1),fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'BH_range_fit.png'), dpi=600)
    # plt.close()
    plt.show()

# print(f"四张回归图已保存至：{output_dir}")
